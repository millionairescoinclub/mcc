...
//
// Bootup the masternode, look for a 100000 MCC input and register on the network
//
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
 		...
 		notCapableReason = "Successfully started masternode.";
            ...
            /* rewards are not supported in Millionairescoinclub.conf */
...
{
    CMillionairescoinclubcoinAddress address(collateralAddress);
