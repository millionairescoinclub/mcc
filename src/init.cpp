...
void Shutdown()
{
    ...
    RenameThread("Millionairescoinclub-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Millionairescoinclub.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Millionairescoinclubd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMillionairescoinclubamount=<n> " + _("Keep N Millionairescoinclub anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Millionairescoinclub is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Millionairescoinclub is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Millionairescoinclub version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Millionairescoinclub server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Millionairescoinclub") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Millionairescoinclub to complete") << "\n";
    ...
    nAnonymizeMillionairescoinclubAmount = GetArg("-anonymizeMillionairescoinclubamount", 0);
    if(nAnonymizeMillionairescoinclubAmount > 999999) nAnonymizeMillionairescoinclubAmount = 999999;
    if(nAnonymizeMillionairescoinclubAmount < 2) nAnonymizeMillionairescoinclubAmount = 2;
    ...
    LogPrintf("Anonymize Millionairescoinclub Amount %d\n", nAnonymizeMillionairescoinclubAmount);
