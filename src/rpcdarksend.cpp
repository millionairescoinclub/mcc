...
void SendMoney(const CTxDestination &address, CAmount nValue, CWalletTx& wtxNew, AvailableCoinsType coin_type=ALL_COINS)
{
    ...
    // Parse Millionairescoinclub address
...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Millionairescoinclubaddress> <amount>\n"
            "Millionairescoinclubaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <Millionairescoinclubaddress> <amount>\n"
            "Millionairescoinclubaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "genkey")
    {
        ...
        return CMillionairescoinclubcoinSecret(secret).ToString();
    }
