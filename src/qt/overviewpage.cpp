...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMillionairescoinclubAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMillionairescoinclubAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMillionairescoinclubAmount = strAnonymizeMillionairescoinclubAmount.remove(strAnonymizeMillionairescoinclubAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMillionairescoinclubAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMillionairescoinclubAmount*COIN) nMaxToAnonymize = nAnonymizeMillionairescoinclubAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMillionairescoinclubAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMillionairescoinclubAmount));
        strAnonymizeMillionairescoinclubAmount = strAnonymizeMillionairescoinclubAmount.remove(strAnonymizeMillionairescoinclubAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMillionairescoinclubAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeMillionairescoinclubAmount == 0){
