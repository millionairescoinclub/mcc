...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMillionairescoinclubAmount"))
        settings.setValue("nAnonymizeMillionairescoinclubAmount", 1000);
    nAnonymizeMillionairescoinclubAmount = settings.value("nAnonymizeMillionairescoinclubAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMillionairescoinclubAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeMillionairescoinclubAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMillionairescoinclubAmount:
            return QVariant(nAnonymizeMillionairescoinclubAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMillionairescoinclubAmount:
            nAnonymizeMillionairescoinclubAmount = value.toInt();
            settings.setValue("nAnonymizeMillionairescoinclubAmount", nAnonymizeMillionairescoinclubAmount);
            emit AnonymizeMillionairescoinclubAmountChanged(nAnonymizeMillionairescoinclubAmount);
            break;
        default:
            break;
        }
